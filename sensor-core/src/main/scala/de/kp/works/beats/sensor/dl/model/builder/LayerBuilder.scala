package de.kp.works.beats.sensor.dl.model.builder
...
import de.kp.works.beats.sensor.dl.model.ModelNames
...
trait LayerBuilder extends SpecBuilder with OptimizerBuilder {
  ...
  def config2Layer(config: Config, kerasVersion: String)
    ...
    val kerasLayer = config.getString(ModelNames.TYPE) match {
      ...
      case "GlobalAveragePooling1D" =>
        if (kerasVersion == "V1")
          config2GlobalAvgPool1D_V1

        else
          config2GlobalAvgPool1D_V2

      /* V1 & V2 */
      case "GlobalMaxPooling1D" =>
        if (kerasVersion == "V1")
          config2GlobalMaxPool1D_V1

        else
          config2GlobalMaxPool1D_V2
   ...
   * the batch dimension).
   */
  def config2BatchNorm: BatchNormalization[Float] = {
    /*
  ...
  def config2Flatten_V2: layers.Flatten[Float] = {
    keras2.layers.Flatten()
  }

  def config2GlobalAvgPool1D_V1: GlobalAveragePooling1D[Float] = {
    keras.layers.GlobalAveragePooling1D()
  }

  def config2GlobalAvgPool1D_V2: layers.GlobalAveragePooling1D[Float] = {
    keras2.layers.GlobalAveragePooling1D()
  }

  def config2GlobalMaxPool1D_V1: GlobalMaxPooling1D[Float] = {
    keras.layers.GlobalMaxPooling1D()
  }

  def config2GlobalMaxPool1D_V2: layers.GlobalMaxPooling1D[Float] = {
