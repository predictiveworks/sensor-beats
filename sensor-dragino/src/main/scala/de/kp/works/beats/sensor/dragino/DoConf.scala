...
class DoConf extends BeatConf {
  ...
  def getProduct:DoProducts.Value = {
    ...
    try {
      DoProducts.withName(product)

    } catch {
      case _:Throwable =>
        throw new Exception(s"Dragino product `$product` not supported.")
    }
   ...
   * tables (see DoDecoder), dynamically configured
  ...
  def getRocksTables: Seq[String] = {
    ...
    val monitoring = {
       ...
       */
      val statics = DoDecoder.tables(product)
      /*
