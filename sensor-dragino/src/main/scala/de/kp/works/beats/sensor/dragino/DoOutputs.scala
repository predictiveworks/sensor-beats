...
/**
 * Dragino output channel for ThingsBoard
 */
class EsBoard(options:DoOptions)
  extends thingsboard.Producer[DoConf](options.toBoard) with DoLogging {
}

/**
 * Implementation of the FIWARE output channel
 */
class MsFiware(options:DoOptions)
  extends fiware.Producer[DoConf](options.toFiware) {
}

/**
 * Implementation of the RocksDB output channel
 */
class MsRocks(options:DoOptions) extends BeatSink {
