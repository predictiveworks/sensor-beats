...
/**
 * Elsys output channel for ThingsBoard
 */
class EsBoard(options:EsOptions)
  extends thingsboard.Producer[EsConf](options.toBoard) with EsLogging {
}

/**
 * Implementation of the FIWARE output channel
 */
class MsFiware(options:EsOptions)
  extends fiware.Producer[EsConf](options.toFiware) {
}

/**
 * Implementation of the RocksDB output channel
 */
class MsRocks(options:EsOptions) extends BeatSink {
