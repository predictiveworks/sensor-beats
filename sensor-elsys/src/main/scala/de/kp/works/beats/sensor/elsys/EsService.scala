...
/**
 * [EsService] is built to manage the various micro
 * services used to provide the REST API and also
 * the multiple output channels
 */
class EsService(config:EsConf) extends BeatService[EsConf](config) with EsLogging {

  override protected var serviceName: String = "EsService"
  private val options = new EsOptions(config)
  ...
  override def buildApiActors(queue: SourceQueueWithComplete[String]): Map[String, ActorRef] = {
    ...
    Map(
       ...
       */
      BEAT_ANOMALY_ACTOR ->
        system.actorOf(Props(new EsAnomActor(config)), BEAT_ANOMALY_ACTOR),
      /*
       * Train time series prediction model
       * & retrieve forecasts
       */
      BEAT_FORECAST_ACTOR ->
        system.actorOf(Props(new EsForeActor(config)), BEAT_FORECAST_ACTOR),
      /*
       * Retrieve sensor inferred readings via SQL query
       */
      BEAT_INSIGHT_ACTOR ->
        system.actorOf(Props(new EsInsightActor(config)), BEAT_INSIGHT_ACTOR),
      /*
       * Retrieve sensor readings via SQL query
       */
      BEAT_MONITOR_ACTOR ->
        system.actorOf(Props(new EsMonitorActor(config)), BEAT_MONITOR_ACTOR),
      /*
       * Retrieve sensor trend via SQL query
       */
      BEAT_TREND_ACTOR ->
      system.actorOf(Props(new EsTrendActor(config)), BEAT_TREND_ACTOR)
  ...
  private def buildInput():Unit = {
    ...
    options.getSource match {
      case HELIUM =>
        val helium = new EsHelium(options)
        helium.subscribeAndPublish()

      case LORIOT =>
        val loriot = new EsLoriot(options)
        loriot.subscribeAndPublish()

      case THINGS_STACK =>
        val stack = new EsStack(options)
  ...
  private def buildOutputs(queue: SourceQueueWithComplete[String]):Unit = {
    ...
    options.getSinks.foreach {
      ...
      case channel@THINGSBOARD =>
         ...
         */
        val props = Props(new EsBoard(options))
        BeatSinks.registerChannel(channel, props)
  ...
  private def buildMonitors(queue: SourceQueueWithComplete[String]):Unit = {
    ...
    val anomMonitor = new AnomMonitor[EsConf](config, anomThreads)
    /*
    ...
    val foreMonitor = new ForeMonitor[EsConf](config,foreThreads)
