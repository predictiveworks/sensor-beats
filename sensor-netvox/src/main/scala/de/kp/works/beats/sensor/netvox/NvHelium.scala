...
class NvHelium(options: NvOptions) extends Consumer[NvConf](options.toHelium) with NvTransform with NvLogging {

  private val BRAND_NAME = "Netvox"
  /**
   * The configured data sinks configured to send
   * Netvox sensor readings to
   */
  ...
  override protected def publish(message: HeliumUplink): Unit = {
    ...
    try {
       ...
       * fields.
       *
       * This restriction is ensured by the Netvox decoders
       * provided with this project
       */
      val product = options.getProduct

      val sensorReadings = NvDecoder.decodeHex(product, new String(decodedPayload), fport)
