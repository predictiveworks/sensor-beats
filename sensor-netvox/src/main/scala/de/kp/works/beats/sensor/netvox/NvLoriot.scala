...
/**
 * The [NvLoriot] input channel focuses on the
 * extraction of the unique device identifier
 * and the provided sensor readings
 */
class NvLoriot(options: NvOptions) extends Consumer[NvConf](options.toLoriot) with NvTransform with NvLogging {
  ...
  override protected def publish(message: LoriotUplink): Unit = {
    ...
    try {
      ...
      val sensorReadings = NvDecoder.decodeHex(product, message.data.get, fport)
