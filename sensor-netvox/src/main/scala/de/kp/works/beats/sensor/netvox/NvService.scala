...
/**
 * [NvService] is built to manage the various micro
 * services used to provide the REST API and also
 * the multiple output channels
 */
class NvService(config:NvConf) extends BeatService[NvConf](config) with NvLogging {

  override protected var serviceName: String = "NvService"
  private val options = new NvOptions(config)
  ...
  override def buildApiActors(queue: SourceQueueWithComplete[String]): Map[String, ActorRef] = {
    ...
    Map(
       ...
       */
      BEAT_ANOMALY_ACTOR ->
        system.actorOf(Props(new NvAnomActor(config)), BEAT_ANOMALY_ACTOR),
      /*
       * Train time series prediction model
       * & retrieve forecasts
       */
      BEAT_FORECAST_ACTOR ->
        system.actorOf(Props(new NvForeActor(config)), BEAT_FORECAST_ACTOR),
      /*
       * Retrieve sensor inferred readings via SQL query
       */
      BEAT_INSIGHT_ACTOR ->
        system.actorOf(Props(new NvInsightActor(config)), BEAT_INSIGHT_ACTOR),
      /*
       * Retrieve sensor readings via SQL query
       */
      BEAT_MONITOR_ACTOR ->
        system.actorOf(Props(new NvMonitorActor(config)), BEAT_MONITOR_ACTOR),
      /*
       * Retrieve sensor trend via SQL query
       */
      BEAT_TREND_ACTOR ->
      system.actorOf(Props(new NvTrendActor(config)), BEAT_TREND_ACTOR)
  ...
  private def buildInput():Unit = {
    ...
    options.getSource match {
      case HELIUM =>
        val helium = new NvHelium(options)
        helium.subscribeAndPublish()

      case LORIOT =>
        val loriot = new NvLoriot(options)
        loriot.subscribeAndPublish()

      case THINGS_STACK =>
        val stack = new NvStack(options)
  ...
  private def buildOutputs(queue: SourceQueueWithComplete[String]):Unit = {
    ...
    options.getSinks.foreach {
      ...
      case channel@THINGSBOARD =>
         ...
         */
        val props = Props(new NvBoard(options))
        BeatSinks.registerChannel(channel, props)
  ...
  private def buildMonitors(queue: SourceQueueWithComplete[String]):Unit = {
    ...
    val anomMonitor = new AnomMonitor[NvConf](config, anomThreads)
    /*
    ...
    val foreMonitor = new ForeMonitor[NvConf](config,foreThreads)
