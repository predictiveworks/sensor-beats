...
object RB11E extends BaseDecoder {
  /*
   * Netvox RB11E is a Wireless Occupancy/Light/Temperature LoRaWAN Sensor.
   *
   * It combines occupancy ,temperature, tamper switch and light sensors
   * that are suitable for detecting human movement indoors, illuminance
   * and temperature.
   */
    ...
    fport match {
      case 6 =>
        ...
        if (bytes(2) == 0x00) {
          ...
          // Illuminance
          val illuminance = (bytes(6) << 8) | bytes(7)
          decoded.addProperty("Illuminance", illuminance)

          // Occupy
          val occupy = bytes(8)
          decoded.addProperty("Occupy", occupy)

          // DisassembledAlarm
          val alarm = bytes(9)
          decoded.addProperty("DisassembledAlarm", alarm)
      ...
      case 7 =>
        ...
        if (bytes(0) == getCmdID("ConfigReportRsp")) {
          ...
          // IlluminanceChange
          val illuminanceChange = (bytes(9) << 8) | bytes(10)
          decoded.addProperty("IlluminanceChange", illuminanceChange)

        }
        else if (bytes(0) == getCmdID("SetIRDisableTimeRsp")) {
          // Status :: Success = 1 | Failure = 0
          val status = if (bytes(2) == 0x00) 1 else 0
          decoded.addProperty("Status", status)
        }
        else if (bytes(0) == getCmdID("GetIRDisableTimeRsp")) {

          // IRDisableTime
          val irDisableTime = (bytes(2) << 8) | bytes(3)
          decoded.addProperty("IRDisableTime", irDisableTime)

          // IRDetectionTime
          val irDetectionTime = (bytes(4) << 8) | bytes(5)
          decoded.addProperty("IRDetectionTime", irDetectionTime)
