...
/**
 * SenseCap output channel for ThingsBoard
 */
class ScBoard(options:ScOptions)
  extends thingsboard.Producer[ScConf](options.toBoard) with ScLogging {
}

/**
 * Implementation of the FIWARE output channel
 */
class ScFiware(options:ScOptions)
  extends fiware.Producer[ScConf](options.toFiware) {
}

/**
 * Implementation of the RocksDB output channel
 */
class ScRocks(options:ScOptions) extends BeatSink {
