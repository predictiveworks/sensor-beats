...
/**
 * [ScService] is built to manage the various micro
 * services used to provide the REST API and also
 * the multiple output channels
 */
class ScService(config:ScConf) extends BeatService[ScConf](config) with ScLogging {

  override protected var serviceName: String = "ScService"
  private val options = new ScOptions(config)
  ...
  override def buildApiActors(queue: SourceQueueWithComplete[String]): Map[String, ActorRef] = {
    ...
    Map(
       ...
       */
      BEAT_ANOMALY_ACTOR ->
        system.actorOf(Props(new ScAnomActor(config)), BEAT_ANOMALY_ACTOR),
      /*
       * Train time series prediction model
       * & retrieve forecasts
       */
      BEAT_FORECAST_ACTOR ->
        system.actorOf(Props(new ScForeActor(config)), BEAT_FORECAST_ACTOR),
      /*
       * Retrieve sensor inferred readings via SQL query
       */
      BEAT_INSIGHT_ACTOR ->
        system.actorOf(Props(new ScInsightActor(config)), BEAT_INSIGHT_ACTOR),
      /*
       * Retrieve sensor readings via SQL query
       */
      BEAT_MONITOR_ACTOR ->
        system.actorOf(Props(new ScMonitorActor(config)), BEAT_MONITOR_ACTOR),
      /*
       * Retrieve sensor trend via SQL query
       */
      BEAT_TREND_ACTOR ->
      system.actorOf(Props(new ScTrendActor(config)), BEAT_TREND_ACTOR)
  ...
  private def buildInput():Unit = {
    ...
    options.getSource match {
      case HELIUM =>
        val helium = new ScHelium(options)
        helium.subscribeAndPublish()

      case LORIOT =>
        val loriot = new ScLoriot(options)
        loriot.subscribeAndPublish()

      case THINGS_STACK =>
        val stack = new ScStack(options)
  ...
  private def buildOutputs(queue: SourceQueueWithComplete[String]):Unit = {
    ...
    options.getSinks.foreach {
      case channel@FIWARE =>
         ...
         */
        val props = Props(new ScFiware(options))
      ...
      case channel@ROCKS_DB =>
         ...
         */
        val props = Props(new ScRocks(options))
        BeatSinks.registerChannel(channel, props)
      ...
      case channel@THINGSBOARD =>
         ...
         */
        val props = Props(new ScBoard(options))
        BeatSinks.registerChannel(channel, props)
  ...
  private def buildMonitors(queue: SourceQueueWithComplete[String]):Unit = {
    ...
    val anomMonitor = new AnomMonitor[ScConf](config, anomThreads)
    /*
    ...
    val foreMonitor = new ForeMonitor[ScConf](config,foreThreads)
