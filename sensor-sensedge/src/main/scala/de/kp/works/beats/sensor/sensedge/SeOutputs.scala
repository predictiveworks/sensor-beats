...
/**
 * Sensedge output channel for ThingsBoard
 */
class SeBoard(options:SeOptions)
  extends thingsboard.Producer[SeConf](options.toBoard) with SeLogging {
}

/**
 * Implementation of the FIWARE output channel
 */
class MsFiware(options:SeOptions)
  extends fiware.Producer[SeConf](options.toFiware) {
}

/**
 * Implementation of the RocksDB output channel
 */
class MsRocks(options:SeOptions) extends BeatSink {
