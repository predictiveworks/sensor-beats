...
/**
 * [Se] is built to manage the various micro
 * services used to provide the REST API and also
 * the multiple output channels
 */
class SeService(config:SeConf) extends BeatService[SeConf](config) with SeLogging {

  override protected var serviceName: String = "SeService"
  private val options = new SeOptions(config)
  ...
  override def buildApiActors(queue: SourceQueueWithComplete[String]): Map[String, ActorRef] = {
    ...
    Map(
       ...
       */
      BEAT_ANOMALY_ACTOR ->
        system.actorOf(Props(new SeAnomActor(config)), BEAT_ANOMALY_ACTOR),
      /*
       * Train time series prediction model
       * & retrieve forecasts
       */
      BEAT_FORECAST_ACTOR ->
        system.actorOf(Props(new SeForeActor(config)), BEAT_FORECAST_ACTOR),
      /*
       * Retrieve sensor inferred readings via SQL query
       */
      BEAT_INSIGHT_ACTOR ->
        system.actorOf(Props(new SeInsightActor(config)), BEAT_INSIGHT_ACTOR),
      /*
       * Retrieve sensor readings via SQL query
       */
      BEAT_MONITOR_ACTOR ->
        system.actorOf(Props(new SeMonitorActor(config)), BEAT_MONITOR_ACTOR),
      /*
       * Retrieve sensor trend via SQL query
       */
      BEAT_TREND_ACTOR ->
      system.actorOf(Props(new SeTrendActor(config)), BEAT_TREND_ACTOR)
  ...
  private def buildInput():Unit = {
    ...
    options.getSource match {
      case HELIUM =>
        val helium = new SeHelium(options)
        helium.subscribeAndPublish()

      case LORIOT =>
        val loriot = new SeLoriot(options)
        loriot.subscribeAndPublish()

      case THINGS_STACK =>
        val stack = new SeStack(options)
  ...
  private def buildOutputs(queue: SourceQueueWithComplete[String]):Unit = {
    ...
    options.getSinks.foreach {
      ...
      case channel@THINGSBOARD =>
         ...
         */
        val props = Props(new SeBoard(options))
        BeatSinks.registerChannel(channel, props)
  ...
  private def buildMonitors(queue: SourceQueueWithComplete[String]):Unit = {
    ...
    val anomMonitor = new AnomMonitor[SeConf](config, anomThreads)
    /*
    ...
    val foreMonitor = new ForeMonitor[SeConf](config,foreThreads)
