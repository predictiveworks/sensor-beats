...
abstract class WeActor extends Actor with WeLogging {
  ...
  protected def dataframeToJson(dataframe:DataFrame):JsonArray = {
    ...
    dataframe.collect.foreach(row => {
      ...
      fields.foreach(field => {
        ...
        ftype match {
          case DoubleType =>
            jRow.addProperty(fname, row.getDouble(fidex))

          case FloatType =>
            jRow.addProperty(fname, row.getFloat(fidex))

          case IntegerType =>
            jRow.addProperty(fname, row.getInt(fidex))

          case LongType =>
            jRow.addProperty(fname, row.getLong(fidex))

          case ShortType =>
            jRow.addProperty(fname, row.getShort(fidex))

          case StringType =>
            jRow.addProperty(fname, row.getString(fidex))

          case _ =>
            jRow.addProperty(fname, row.get(fidex).asInstanceOf[String])
